...
def test_filepath_converter_returns_full_path_if_provided_with_just_filename(temp_filepath):
    converted_filepath = filepath_converter(str(temp_filepath.name), data_directory=str(temp_filepath.parent))
...
def test_scope_converter_throws_exception_if_object_not_found():
    ...
    assert  str(excinfo.value) == 'Object of name test_object does not exist within scope.'
